<?php
class Wsu_Opc_Helper_Paypal extends Mage_Customer_Helper_Data {
	
	const RETURN_URL = 'onepage/paypal/return';
	const ENABLED = 'wsu_opc/paypallogin/status';
	const SANDBOX_MODE = 'wsu_opc/paypallogin/sandbox';
	const CLIENT_ID = 'wsu_opc/paypallogin/clientid';
	const CLIENT_SECRET = 'wsu_opc/paypallogin/secret';
	...
	public function __construct() {
		$this->_sandbox = Mage::getStoreConfig ( self::SANDBOX_MODE );
		$this->_clientId = Mage::getStoreConfig ( self::CLIENT_ID );
		$this->_clientSecret = Mage::getStoreConfig ( self::CLIENT_SECRET );
	}
	...
	public function getAuthUrl() {
		...
		$authUrl = sprintf ( "%s?client_id=%s&response_type=code&scope=%s&redirect_uri=%s&nonce=%s&state=%s", $this->getAuthorizationEndPoint (), Mage::getStoreConfig ( self::CLIENT_ID ), urlencode ( $this->_scopes ), urlencode ( Mage::getBaseUrl ( Mage_Core_Model_Store::URL_TYPE_LINK, true ) . self::RETURN_URL ), $token->nonce, $token->state );
		return $authUrl;
	}
	...
	public function linkAccounts($_profile, $customer) {
		...
		$paypalCustomer = Mage::getModel ( 'wsu_opc/paypal_customer' )->load ( $customerId, 'customer_id' );
